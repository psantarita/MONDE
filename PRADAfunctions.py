...
def myCurve(x0=0,y0=0,np=50,A=100000,S=1,side="lower"):
...
def getRandTheta(dList,x,y):
    """Gets a list with r and theta for a given a point"""
    rAndThetaList=[]
...
def getSignals(dList,x,y,petalSignals=False):
    """Gets the signals list for a given a point"""
    ...
    for e in rTList:
        ...
        if petalSignals:
            ...
            S=A*exp(-r/lamb)*abs(sin(pi*(theta-thetaC)/(pi-2*thetaC)))/r**gamma
